...
priority_queue<point,  vector<point>,  gt> point_to_print;
...
void process_point()
{
   ...
   point p = points.top();
   point_to_print.push(points.top());
   points.pop();
...
void allegro(){
   ...
   while (!point_to_print.empty()){
      al_draw_filled_circle((float)point_to_print.top().x, (float)point_to_print.top().y, (float)R, al_map_rgb_f(1,1,1));
      point_to_print.pop();
   }
   ...
   for (i = output.begin(); i != output.end(); i++) {
      point p0 = (*i)->start;
      point p1 = (*i)->end;
      al_draw_line((float)p0.x, (float)p0.y, (float)p1.x, (float)p1.y, al_map_rgb_f(0,1,0),R);
   }
